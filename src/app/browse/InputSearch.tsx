...
const InputSearch: React.FC<InputProps> = ({token}) => {
  ...
  return (
    <div>
      <form
      onSubmit={onSearch}
      className='flex flex-row justify-start p-2 relative'>
          <input
            placeholder='Search...'
            value={search}
            onChange={(e)=>setSearch(e.target.value)}
            className='text-black py-3 px-5 w-full rounded-full focus:outline-none focus:border-[#5182ff76] focus:ring-1 focus:ring-[#5182ff] bg-blue-100 focus:bg-gradient-to-br from-blue-900 to-slate-200 focus:text-[#fff] focus:placeholder:text-zinc-100/50'
          />
